...
function validateFlightConnections(data, settings, results) {
  ...
  connections.forEach(connection => {
    ...
    if (transferMinutes < 0) {
      const severity = is_critical ? 'error' : 'warning';
      results[severity + 's'].push({
        severity,
...
function validateAircraftType(aircraftType, aircraftTypes, record, index, results) {
  const aircraft = aircraftTypes.find(a => 
    a.IATACode === aircraftType || a.ICAOCode === aircraftType || a.AircraftType === aircraftType
  );
  ...
  if (!aircraft) {
    ...
    if (similarTypes.length > 0) {
      results.warnings[results.warnings.length - 1].suggestedFix = similarTypes[0];
    }
  }
}
...
function findSimilarAircraftTypes(type, aircraftTypes) {
  ...
  return aircraftTypes
    .filter(a => {
      const cleanIATA = (a.IATACode || '').replace(/[^a-zA-Z0-9]/g, '');
      const cleanICAO = (a.ICAOCode || '').replace(/[^a-zA-Z0-9]/g, '');
